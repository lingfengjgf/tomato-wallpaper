	...
	<view class="">
		<view class="" v-show="!loadShow">
			...
			<view class="u-p-t-20 banner">
				<u-swiper :effect3d="true" height="280" bg-color="#FFFFFF" name="image" border-radius="16" effect3d-previous-margin="0" :list="bannerList" @click="bannerClick"></u-swiper>
			</view>
					...
					<view class="u-flex u-row-left u-p-l-40 u-p-r-40">
						<!-- <block> -->
							<view class="tab" v-for="(item, index) in hotList" :key="index" @click="detail(item._id, 0, item.images)">
								<image :src="item.image+'?x-oss-process=image/resize,w_200/quality,q_100'" style="width: 215rpx;height: 350rpx;" mode="aspectFill"></image>
					...
					<block v-for="(item, index) in coverList" :key="index">
						<block v-for="(item_, index_) in item.images" :key="index_">
							...
							<view class="tab" @click="detail(item._id, index_, item.images)">
								<image :src="item_+'?x-oss-process=image/resize,w_200/quality,q_80'" lazy-load style="width: 215rpx;height: 350rpx;" mode="aspectFill"></image>
								<!-- <view class="array">{{item.images.length}}</view> -->
							</view>
						</block>
					</block>
...
.div3{height: 40rpx;width: 88%;background: #ffffff59;border-radius: 10px 10px 0px 0px;position:absolute;bottom: 10px;left: 6%;right: 8%;}	
	...
	.tags{
		...
		margin-right: 20rpx;
		white-space:nowrap;
	}	.banner{width: 100%;padding: 0 40rpx;}
	.tag-active{
